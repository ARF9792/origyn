 ...
 */
export async function applyDocumentImpact(documentId: string): Promise<ImpactResult> {
  ...
  const allAnswers = await listAllAnswers();
 ...
 */
export async function getDocumentImpactView(documentId: string): Promise<{
  claimIds: string[];
  answerIds: string[];
  claimCount: number;
  answerCount: number;
